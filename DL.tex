...
\begin{equation}
	...
	\end{bmatrix};
\end{equation}
In \textbf{vector calculus}, the derivative of a vector function $\mathbf{y}$ with respect to a vector $\mathbf{x}$ whose components represent a space is known as the \textbf{Jacobian matrix}. \par
The \textbf{directional derivative} of a scalar function $f(\mathbf{x})$ of the space vector $\mathbf{x}$ in the direction of the unit vector $\mathbf{u}$ is defined using the gradient as follows. \par
