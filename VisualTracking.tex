...
%--------------------------------------------------
\subsubsection{Matrix Derivatives \citep{nasrabadi2007pattern}}
\begin{equation} \label{eq:matrixderivativ1}
	\frac{\partial}{\partial \bm{x}}(\bm{x^Ta}) = 
	\frac{\partial}{\partial \bm{x}}(\bm{a^Tx})= \bm{a}
\end{equation}
...
%--------------------------------------------------
\subsubsection{Conjugate Gradient \citep{shewchuk1994introduction}} \label{ch:conjugategradient}
...
%-----------------------------------------------
\subsection{Spatially Regularization $W$ \citep{danelljan2015learning}}
...
$f^1, \cdots, f^C$, where $C < D$. Then the filter for feature layer $d$ could be constructed as: 
$f^d = \sum^C_{c=1} p_{d,c}f^c$. \par
Define: $P=(p_{d,c})_{D \times C}$, then the filter could be written as: $(Pf)_{D \times 1}$, so we have 
the factorized convolution operator from (\ref{eq:score}), \par
\begin{equation}
	S_{Pf}\{x\}= Pf*J\{x\}=\sum_{c,d} p_{d,c} f^c * J_d\{x^d\} = f*P^TJ\{x\}
\end{equation} \par

\begin{equation} \label{eq:targetFactorized}
	E(f, P)=\norm{\hat{z}^TP\hat{f}-\hat{y}_j}^2_{l^2}
...
\end{split}\end{align} 
where $\hat{f}_{i, \Delta} = \hat{f}_i + \Delta \hat{f}$. \par
Substitute (\ref{eq:approTayor}) into (\ref{eq:targetFactorized}):
\begin{equation}
